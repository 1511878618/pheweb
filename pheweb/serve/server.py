...
def region_page(phenocode: str, region: str):
    ...
    return render_template(
        ...
        pheno=pheno,
        region=region,
        show_cond=conf.should_show_cond(),
        tooltip_lztemplate=parse_utils.tooltip_lztemplate,
    )
...
def api_gene():
    ...
    if not isinstance(filter_param, str):
        abort(404)
    m = re.match(
        r".*chrom eq '(.+?)' and start le ([0-9]+) and end ge ([0-9]+)", filter_param
    )
    if not m:
        abort(404)
    else:
        chrom, pos_start, pos_end = m.group(1), int(m.group(3)), int(m.group(2))
        res = jsonify(get_gene_region(chrom, pos_start, pos_end))
        if res.data is None:
            return (
                jsonify(
                    {
...
def get_conditional():
    ...
    phenocode = request.args.get("phenocode")
    region = f"{chr}:{start}-{end}"
    res = jsonify(query_conditional_to_json(region, phenocode))
    # if res.data is None:abort(404)
    return res
...
def get_cond():
    ...
    fileName = re.findall(r"(?<=topLociCond).+", file)[0]
    # return fileName
    return jsonify(query_cond_file(region, fileName))
    ...
    def oauth_callback_google():
        ...
        try:
            (
                username,
                email,
            ) = google_sign_in.callback()  # oauth.callback reads request.args.
        except Exception as exc:
