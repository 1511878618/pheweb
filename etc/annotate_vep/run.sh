...
#note carefully to change binary path of ./vep
...
python_exe="/home/xutingfeng/software/install_software/anaconda3/envs/pheweb_wanglab/bin/python"
#python_exe="python3"
...
if ! [[ $parallel = "yes" ]]; then
    ...
    zcat input.vcf.gz | grep -v '^#' | split --lines=$((3 * 1000 * 1000)) - split_
    for file in split_*; do
        zcat input.vcf.gz | head -n3 >"vep_data/input/$file"
...
docker run -v "$PWD/vep_data":/opt/vep/.vep ensemblorg/ensembl-vep perl /opt/vep/src/ensembl-vep/INSTALL.pl -a cfp -s homo_sapiens -y "$build" -g all # Do we really need `-g all`?

if ! [[ $parallel = "yes" ]]; then
    docker run -v "$PWD/vep_data":/opt/vep/.vep ensemblorg/ensembl-vep /opt/vep/src/ensembl-vep/vep --input_file=/opt/vep/.vep/input/input.vcf.gz --output_file=/opt/vep/.vep/output.tsv --force_overwrite --compress_output=gzip --cache --offline --assembly="$build" --regulatory --most_severe --check_existing
    mv vep_data/output.tsv out-raw-vep.tsv

else
    for f in vep_data/input/split_*; do
        name=$(basename "$f")
        docker run -v "$PWD/vep_data":/opt/vep/.vep ensemblorg/ensembl-vep /opt/vep/src/ensembl-vep/vep --input_file=/opt/vep/.vep/input/$name --output_file=/opt/vep/.vep/output-$name.tsv --force_overwrite --compress_output=gzip --cache --offline --assembly="$build" --regulatory --most_severe --check_existing &
    done
